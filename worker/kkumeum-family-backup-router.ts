...
export async function handleKkumeumFamilyBackupApi(request: Request, env: KkumeumFamilyBackupRouterEnv): Promise<Response | null> {
  if (new URL(request.url).pathname !== "/api/kkumeum/admin/family-backups") return null;
  try {
    ...
    if (request.method === "GET") return json({ manifests: await listKkumeumFamilyBackupManifests(env.FAMILY_DB, context) });
    if (request.method === "POST") { sameOrigin(request); return json({ manifest: await createKkumeumFamilyBackupManifest(env.FAMILY_DB, env.FAMILY_FILES, context) }, { status: 201 }); }
