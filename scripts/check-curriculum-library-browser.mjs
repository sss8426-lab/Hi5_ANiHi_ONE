...
try{
  ...
  await ctx.route('**/api/**',async route=>{
    ...
    if(u.pathname.startsWith('/api/data-core/curriculum')){
      ...
      const match=u.pathname.match(/folders\/syn-(basic|advanced|admission)-(\d+)$/),stage=match?.[1]||u.searchParams.get('stage'),index=match?Number(match[2]):null,lesson=u.searchParams.get('lesson');
      const list=folders(stage),folder=index===null?null:list[index];
      return route.fulfill({json:{family:'content',stage,folder,breadcrumbs:folder?[folder]:[],folders:folder?[]:list,totalFolders:list.length,totalPages:list.length*4,
  ...
  for(const width of widths){
    ...
    await page.goto(`${base}/data-core/curriculum/content`);
    await page.getByText('5개 수업',{exact:true}).nth(2).waitFor();await noOverflow();
    for(const img of await page.locator('.curriculum-stage-card img').all()){await img.scrollIntoViewIfNeeded();await img.evaluate(i=>i.decode());}
    await page.screenshot({path:`${out}/stage-selection-${width}.png`,fullPage:true});
    for(const stage of Object.keys(stages)){
      ...
      assert.equal(await page.locator('.lesson-canvas img').evaluate(i=>getComputedStyle(i).objectFit),'contain');
      ...
      await page.goForward();await page.getByText('2 / 4',{exact:true}).waitFor();
      await page.goBack();await page.getByText('1 / 4',{exact:true}).waitFor();
      ...
      await page.locator('.lesson-breadcrumb a').filter({hasText:stages[stage]}).click();await page.locator('.lesson-card').first().waitFor();
  ...
  assert.equal(await page.locator('.curriculum-print-sheet').count(),4);
  await page.goto(`${base}/data-core/curriculum/content`);await page.getByText('6개 수업',{exact:true}).nth(2).waitFor();
