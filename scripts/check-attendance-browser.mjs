...
try {
  ...
  role='MASTER';await page.reload();await page.locator('#atFile').waitFor();await page.locator('#atFile').setInputFiles({name:'SYNTHETIC.xlsx',mimeType:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',buffer:source});await page.locator('#atForm:visible').waitFor();await page.locator('#atCampus').selectOption('synthetic-b');await page.locator('#atFile').waitFor();assert.equal(await page.locator('#atForm').isVisible(),false);
