...
const run = (args) => new Promise((resolve, reject) => {
  execFile(process.platform === 'win32' ? 'npx.cmd' : 'npx', ['wrangler', 'd1', 'execute', 'DB', '--config', 'dist/server/wrangler.json', '--remote', '--yes', ...args], { windowsHide: true }, (error, stdout, stderr) => {
...
try {
  ...
  const command = `BEGIN; INSERT OR IGNORE INTO organizations (id, slug, name, status, created_at, updated_at) VALUES ('org-hi5-anihi', 'hi5-anihi', 'HI5·ANiHi', 'active', ${sql(now)}, ${sql(now)}); INSERT INTO users (id, email, display_name, status, created_at, updated_at) VALUES (${sql(userId)}, NULL, ${sql(loginId)}, 'active', ${sql(now)}, ${sql(now)}); INSERT INTO auth_accounts (id, user_id, login_id, password_hash, password_salt, password_iterations, status, must_change_password, failed_login_count, created_at, updated_at) VALUES (${sql(accountId)}, ${sql(userId)}, ${sql(loginId)}, ${sql(hash)}, ${sql(salt)}, ${iterations}, 'active', 0, 0, ${sql(now)}, ${sql(now)}); INSERT INTO memberships (id, organization_id, campus_id, user_id, role, created_at, updated_at) VALUES (${sql(membershipId)}, 'org-hi5-anihi', NULL, ${sql(userId)}, 'SUPER_ADMIN', ${sql(now)}, ${sql(now)}); COMMIT;`;
