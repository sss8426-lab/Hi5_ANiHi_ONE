...
export async function cloudflare(configPath) {
  ...
  async function raw(path,options={}) {
    for(let attempt=0;attempt<5;attempt++){
      let response;
      try{response=await fetch(base+path,{...options,headers:{Authorization:`Bearer ${credentials.token}`,...options.headers},signal:AbortSignal.timeout(120000)});}catch{if(attempt===4)throw Error('Cloudflare 연결 실패. 원문은 출력하지 않았습니다.');}
