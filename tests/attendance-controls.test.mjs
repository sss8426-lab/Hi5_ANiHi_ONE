...
test('attendance is upload / month / generate; no technical mapping or original preview UI',async()=>{
  ...
  assert.doesNotMatch(script,/atMapping|data-map|atSource|atReadRows|atYear|atConfirm|양식 영역 확인|원본 미리보기/);
  assert.doesNotMatch(script,/fetch\(|localStorage|indexedDB|console\./);
  assert.match(script,/analyzeWorkbook\(template,filename\)/);assert.match(script,/generateWorkbook\(template,analysis/);
});
