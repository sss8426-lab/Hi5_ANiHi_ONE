...
test('uniform decorative color is not a lesson schedule',()=>{
  ...
  putValue(grid.cells.get('C5'),'화',sheet);
  const t=openTemplate(zipSync({...s.t.entries,'xl/worksheets/sheet1.xml':strToU8(xml(sheet)),'xl/styles.xml':strToU8(xml(styles))}),env);
  const b=analyzeWorkbook(t).sheets[0].studentBlocks[0];assert.deepEqual(b.channels[0].slots,[{slot:0,weekdays:[2]}]);
