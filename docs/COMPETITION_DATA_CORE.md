...
- The selection bar offers select-all/deselect-all before any individual selection. It affects only loaded files in the selected folder that the current user may delete. Empty/read-only states hide it, loading/deletion disables it, and changing folders clears selection. Selection itself makes no API writes; deletion still requires the existing confirmation.
