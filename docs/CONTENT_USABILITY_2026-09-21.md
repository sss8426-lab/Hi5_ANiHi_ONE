...
| 항목 | 전 | 후 |
|---|---:|---:|
| 첫 폴더 사용 가능 | 1,944ms | 335ms |
| 폴더 재방문 중앙값 | 872ms | 323ms |
| 폴더 재방문 p95 | 1,392ms | 367ms |
| 첫 파일 목록 | 834ms | 908ms |
| 첫 썸네일 | 1,109ms | 1,096ms |
| 측정 중 API 요청 | 234 | 231 |
| 원본 이미지 요청 | 0 | 0 |

폴더 대기는 크게 감소했으나 첫 사진 목록 속도 자체는 개선으로 주장하지 않음.
30회 이동 후 cache 6개/1,200 bytes, pending active0. 표본은 작고 운영 부하는 별도.
