...
- 로컬 코드 검증은 별도로 실행해야 하며, 실제 운영환경 성공으로 간주하지 않는다.
...
2. 운영 배포 URL에서 마스터 이메일로 로그인한다.
3. `/data-core/readiness`에서 전체 진단을 실행한다.
4. D1/R2 진단이 모두 성공하는지 확인한다.
5. `/data-core`에서 작은 테스트 파일을 업로드한다.
6. 파일 열기, 검색, 휴지통 이동, 복원을 확인한다.
7. `/data-core/operations`에서 백업을 생성하고 manifest를 확인한다.
8. 입시데이터 동기화를 실행하고 `/data-core/roadmap` 연결을 확인한다.
9. `/data-core/content`에서 블로그/인스타 초안을 각각 생성하고 같은 DATA CORE fileId를 연결해 본다.
