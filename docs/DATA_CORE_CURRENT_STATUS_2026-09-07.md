...
## 학원 일정 상세·검색 개선 (2026-09-21, 작업 브랜치)

업무홈/상담홈의 상세창, 날짜/일정 클릭 분리, 선택 월 검색·범위/유형 필터,
월간/목록·연월 이동, 시간/장소, 복사, 한국 기준 오늘/7일 요약을 공통 구현했다.
기존 최신100건 이후 날짜 필터 제한은 SQL 권한/기간 필터와 keyset pagination으로
대체한다. 기존 날짜 호환·실패 입력 보존·중복 저장 방지·공모전 observer를 유지한다.
검증 및 한계는 [일정 상세 개선](CALENDAR_DETAILS_2026-09-21.md)과 해당 PR evidence 참조.
운영 데이터/migration/R2 변경 없음. 별도 승인 전 main 병합·production 배포하지 않는다.

## 출석부 주말 수업 횟수 확인 (2026-09-21)

수업요일 확인창에 토요일/일요일 각각 최대 3타임 선택을 추가했다. 학생별 선택 횟수를
생성 Excel에 반영하고, 기존 다중 행/다중 열과 주간 생략 양식 adapter를 재사용한다.
원본 날짜칸 고정 상태에서 칸이 부족하면 누락시키지 않고 안내한다. 원본 workbook과
자동 인식된 다른 학생 설정은 보존한다. DB/R2/권한 변경 없음.
검증 범위는 [주말 수업 선택](ATTENDANCE_WEEKEND_SELECTION_2026-09-21.md)과 PR evidence 참조.

## 일정 등록·수정 복구 (2026-09-21)

홈의 metadata 날짜와 신규 등록 API 계약 불일치를 수정했다. 종료일 제거,
캠퍼스 초기값, 중복 저장 방지, 오류 시 입력 보존을 보강하고 공모전 달력
감시자 중복 등록을 제거한다. 같은 달 저장은 확정된 응답으로 즉시 반영하며
월 이동의 이전 요청을 취소한다. 기존 일정/DB/R2/권한은 변경하지 않는다.
검증 범위와 제한은 [일정 복구 기록](CALENDAR_ENTRY_REPAIR_2026-09-21.md),
배포 결과는 해당 PR evidence를 따른다.
...
## 인스타 공식 로고·승인 제작 (2026-09-21)

사용자 제공 공식 마크 3종과 중앙 캠퍼스 표시명 함수를 추가했다. 계정/폴더의 캠퍼스 원본명은 유지한다.
학생 작품은 전체 contain 배치하며 텍스트/로고는 작품 외부에 합성한다. 2160x2700 마스터와
승인된 1080x1350 출력은 기존 FILES/file_objects/derivative 구조를 재사용한다.
초안 변경 시 이전 승인은 무효이며 서버에서 현재 버전과 권한을 재검사한다. 별도 DB migration은 없다.
AI 이미지 편집은 별도 동의가 확인된 보조 이미지로 제한하며 기존 OpenAI key/model을 유지한다.
로고 픽셀/인체/개인정보/숫자·날짜 의미 대조는 사람 검수가 필요하다. 자동 게시/자동 사실 보증은 없다.
구현 범위와 로컬·Preview·실제 provider 검증 구분은 `docs/INSTAGRAM_BRAND_REVIEW_2026-09-21.md` 참조.

## 자료보관함 업로드 대기 개선 (2026-09-21)

원본 전송과 미리보기 준비를 겹쳐 처리하고, 한 장의 썸네일 서버 응답 때문에 다음 이미지 decode가 기다리던 병목을 제거했다.
decode는 한 장씩, 파일 작업은 기존 최대 3개로 제한하며 서버 multipart 재파싱을 없앴다. 미리보기 실패/15초 timeout은 원본 성공을 유지한다.
원본 품질/bytes/파일명과 캠퍼스/private 권한, 기존 D1/R2는 변경하지 않는다. 고정 지연 로컬 8장 비교는 약 59% 단축이나 운영 측정값은 아니다.
변경 파일의 기존 lint 오류와 의존성 audit 12건(중간 4/높음 8)은 별도 잔여 부채다. 검증 경계는 `docs/LIBRARY_UPLOAD_PERFORMANCE_2026-09-21.md` 참조.

## 인스타 캠퍼스 로고·PNG 저장 호환 보완 (2026-09-21)

마스터의 캠퍼스 선택 및 폴더 이동 시 공식 로고 5종을 갱신한다. 늦게 도착한 이전 캠퍼스 응답은 무시한다.
PNG/JPG/JPEG/WebP/GIF/AVIF/BMP 원본의 자동 비례 배치를 지원하고, 완성 캔버스는 별도 브라우저 Worker에서
기존 fast-png 코덱으로 2160x2700 RGB PNG로 직렬화한다. 서버의 CRC/크기/압축 해제 안전 검사는 유지한다.
검수 렌더만 최대 16MiB로 제한하며 원본, AI 입력 예산, 다른 파생/썸네일 제한은 변경하지 않는다.
입력 안전 한도 및 실제 사용자 오류 재현의 한계, 합성 브라우저/workerd 검증은 `docs/INSTAGRAM_CAROUSEL_2026-09-21.md` 참조.
기존 API 키와 D1/R2 및 캠퍼스/private 권한 유지. 운영 배포 여부는 PR/최종 보고에서 별도 확인한다.

## 인스타 AI 사진 보정 중간 이미지 처리 (2026-09-21)

AI 결과를 서버에서 먼저 2160x2700으로 확대하면서 8MiB 제한에 걸리는 오류를 상세 합성 PNG로 재현했다.
carousel-v2는 보정 중간 이미지의 검증된 원래 해상도를 유지하고, 브라우저의 최종 로고 합성에서만 크기를 맞춘다.
중간 이미지의 실제 크기와 처리 표식을 기록하며, 보호된 파생 파일 조회는 해당 형식만 추가 허용한다.
원본 접근 권한·캠퍼스·삭제·출처 검사, 기존 4:5 API, 최종 출력 규격, 키와 모델은 유지한다.
재현 수치 및 실제 provider 검증 경계는 `docs/INSTAGRAM_AI_PHOTO_FIX_2026-09-21.md` 참조.
사용자의 실제 AI 제작 성공 확인 후, 저장된 최신 결과의 미리보기는 로컬 PNG를 재사용하여 중복 다운로드를 없앴다.
단 한 장만 메모리에 유지하고 새 제작/선택 변경 시 해제한다. 과거 결과 조회는 기존 인증 API를 유지한다.

## 인스타·블로그 사용성 및 공통일정 통합 (2026-09-21)

PR #242에서 폴더/파일 단계별 표시, bounded authenticated thumbnail cache, 로고 없음, 저장 상태/다운로드,
2열 고정 문구 및 비동기 AI 월간 예산 요약을 추가한다. 공식 비용/예산은 별도 관리 API 권한과 프로젝트 지정이 필요하며 미확인 값을 0으로 표시하지 않는다.
기존 `feat/calendar-details` 작업을 보존 통합하고 상세·검색·필터·목록·복사 및 권한/페이지네이션을 검증한다.
운영 자료/키/모델 변경 및 main 병합/production 배포는 수행하지 않는다.
측정 결과와 검증 경계: `docs/CONTENT_USABILITY_2026-09-21.md`.
