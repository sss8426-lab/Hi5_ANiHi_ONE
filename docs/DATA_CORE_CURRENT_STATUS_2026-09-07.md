...
표시 보완: 파일 선택 전 숨겨졌던 연도·월 선택을 항상 표시하고 이전/다음 달·이번 달 이동을 제공한다. 학생·수업요일·보강일/휴원일/다운로드·인쇄도 화면에 유지하며 필요한 Excel/생성 결과가 없을 때만 비활성화한다. 설정 변경 시 오래된 결과 다운로드를 막고, 잘못된 파일 선택 후 정상 파일로 복구할 수 있다. 기존 원본 보존 엔진/API/DB/R2는 변경하지 않는다.

**업무홈의 꿈이음 바로 아래 출석부**(`/data-core/work/attendance`)에 기존 XLSX 양식의 월 변경 도구를 배치한다. 왼쪽 업무 메뉴에도 꿈이음 다음에 표시하며 꿈이음 내부에는 삽입하지 않는다. 기존 출석체크 바로가기는 독립 업무 화면으로 이동한다. 원본 ZIP 패키지를 복사하고 선택한 시트의 날짜·요일·수업일 표시를 수정하며 31개 날짜 열과 학생 순서를 유지한다. 브라우저 로컬 처리이며 학생 DB 동기화나 출석 기록 서버 저장 기능은 아니다. 20/30명 합성 양식 한 장, 60명 세로 두 장, 윤년·2월, 세로형 A4, 다운로드/원본 해시와 Chromium PDF를 검증했다. 실제 사용자 Excel/물리 프린터 검수와 배포 상태는 [출석부 양식 보존](ATTENDANCE_TEMPLATE_PRESERVATION_2026-09-14.md) 및 PR evidence에서 구분한다. 새 API/DB migration/R2 변경은 없다.
