...
꿈이음의 출석체크 메뉴에 기존 XLSX 양식의 월 변경 도구를 연결한다. 원본 ZIP 패키지를 복사하고 선택한 시트의 날짜·요일·수업일 표시를 수정하며 31개 날짜 열과 학생 순서를 유지한다. 브라우저 로컬 처리이며 학생 DB 동기화나 출석 기록 서버 저장 기능은 아니다. 20/30명 합성 양식 한 장, 60명 세로 두 장, 윤년·2월, 세로형 A4, 다운로드/원본 해시와 Chromium PDF를 검증했다. 실제 사용자 Excel/물리 프린터 검수와 배포 상태는 [출석부 양식 보존](ATTENDANCE_TEMPLATE_PRESERVATION_2026-09-14.md) 및 PR evidence에서 구분한다. 새 API/DB migration/R2 변경은 없다.
