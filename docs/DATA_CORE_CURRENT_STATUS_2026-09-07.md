...
## 24. 출석부 주말 다중 수업칸 자동 인식 및 월별 재배치 (2026-09-14)

업로드된 출석부에서 토/일이 물리적으로 2~3칸을 차지하는 병합/반복 헤더 구조를 자동 인식하고, 요일별 칸
수를 하드코딩하지 않고 원본 1~28일 다수결로 추론한다(`weekdaySlotPolicy`). 다음 달 생성 시에는 원본의
날짜 번호가 아니라 **대상 월의 실제 달력 + 추론된 정책**으로 물리 열을 새로 구성하고(`targetCalendarColumns`),
달력 폭이 달라지면 학생 정보 열은 그대로 두고 오른쪽 꼬리 영역(병합/조건부서식/하이퍼링크/인쇄영역 포함)만
안전하게 늘리거나 줄인다(`reshapeCalendar`). 지원 불가한 구조(수식/도형/표 등)가 있는 상태에서 재구성이
실제로 필요하면 파일을 깨뜨리지 않고 한국어 오류로 막는다. 기존 `dateColumns`/`channels[].slots[]` 구조를
그대로 확장했고, 다중 slot 요일의 0번 slot에서 학생의 실제 slot이 아니라 행 전체 요일 목록을 잘못 쓰던
기존 버그도 함께 고쳤다. 새 UI 컨트롤은 추가하지 않았다. 출석부 외 다른 기능은 변경하지 않았다. 세부
내용과 테스트는 `docs/ATTENDANCE_MULTI_SLOT_WEEKENDS_2026-09-14.md`를 따른다.
