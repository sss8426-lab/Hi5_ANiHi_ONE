...
- ZIP은 기존 fflate 0.8.3으로 1KiB 입력 조각씩 실제 해제 크기를 검사하므로 크기 필드 위조도 차단한다. 복잡한 엑셀 차트/그림/서식과 모든 PDF 압축 스트림의 메모리를 완전히 예측하는 기능은 없다. 파서는 별도 Worker로 실행하고 종료 가능하게 한다. 원본과 완전히 같은 편집기 렌더링을 보장하지 않는다.
...
## 변경 전후 성능 (로컬 합성 데이터, 3회 중앙값)

기준은 main `8fecdc51`이며 최종 측정 스크립트는 클릭 이벤트부터 원본 미리보기 준비까지 잰다. Playwright의 스크롤/클릭 준비 시간은 제외한다.
...
| 첫 썸네일 | 610ms | 639ms |
| 최근 카드 | 338ms | 332ms |
| 이미지 첫 미리보기 | 25ms | 402ms |
| 이미지 재방문 | 6ms | 97ms |
| 첫 표시 시 API | 13 | 13 |
| 첫 표시 시 D1 prepare | 78 | 81 |
| 첫 표시 시 R2 호출 | 5 | 6 |

모든 항목이 빨라진 것은 아니다. 이전 갤러리는 이미 표시한 이미지 캐시를 즉시 보여주지만 새 뷰어는 매번 권한을 재확인한다. 최초 HEAD 361ms(썸네일 요청과 동시), 재방문 HEAD 89ms, 최초 이미지 헤더 파서 38ms가 측정됐다. 권한 취소 후 캐시 재노출을 방지하기 위한 비용이며 검사를 생략하지 않는다. R2 횟수는 썸네일 동시 로딩의 관찰 시점 영향을 받는다. 업로드/이동/반복 탐색 결과와 함께 해석하며 인터넷 운영 성능 향상을 보장하는 수치로 사용하지 않는다.
