...
## Shared Campus Presentation (2026-09-14)

The folder API now presents canonical campus labels and display order from `worker/campus-directory.ts`. Library, Blog and Instagram use `DataCoreLibraryClient.folderGroups` for identical root groups and nested folder names/order. Campus roots form one ordered `캠퍼스` group; HQ defaults and user-created roots remain separate. No folder IDs, nesting, stored names, ACLs or files are rewritten. The exact legacy test-campus projection is also hidden by the common campus-selector API. Its authorized recovery URLs and stored dependencies remain intact. See [presentation and read-only inventory](CAMPUS_LIBRARY_PRESENTATION_2026-09-14.md).
